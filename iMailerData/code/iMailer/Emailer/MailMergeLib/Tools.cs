...
{
	/// <summary>
	/// Tools used by classes of Infolancers.iMailer.Emailer.
	/// </summary>
	public static class Tools
	{
		...
		/// <summary>
		/// Gets the string representation of the Mime content of a MailMessage
		/// as it would be sent with SmtpClient.DeliveryMethod = SmtpDeliveryMethod.Network.
		/// Note: Automatic identifiers and the Date Header inside the message will differ with each invocation.
		/// </summary>
		/// <param name="msg">MailMessage msg</param>
		/// <param name="encoding">Encoding</param>
		/// <returns>Mime content as string</returns>
		/// <remarks>
		/// Uses System.Reflection with System.Net.Mail.MailWriter, which makes the method fragile
		/// for code changes of the .NET Framework.
		/// </remarks>
		public static string GetMailAsMimeString(MailMessage msg, Encoding encoding)
		{
			// create a memory stream with expandable capacity
			var stream = new MemoryStream(100);
			Assembly asm = Assembly.GetAssembly(typeof (MailMessage));
			Type mailWriterType = asm.GetType("System.Net.Mail.MailWriter", true);
			object mailWriter = Activator.CreateInstance(mailWriterType,
			                                             BindingFlags.NonPublic | BindingFlags.Instance, null,
			                                             new object[] {stream}, null);

			const BindingFlags flags = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance;

			// invoking "Send" will write into MemoryStream (nothing else)
			msg.GetType().GetMethod("Send", flags).Invoke(msg, new[] {mailWriter, true});

			return Stream2String(stream, encoding);
		}


		/// <summary>
		/// Calculates the size of a MailMessage
		/// </summary>
		/// <param name="msg"></param>
		/// <param name="encoding"></param>
		/// <returns>Size of a message in bytes.</returns>
		public static long CalcMailSize(MailMessage msg, Encoding encoding)
		{
			if (msg == null)
				return -1;

			// return the length of the sring with final CRLF length appended
			return GetMailAsMimeString(msg, encoding).Length + 2;
		}
	}
}